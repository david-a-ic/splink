...
class YellowbrickDataFrame(SplinkDataFrame):
    linker: YellowbrickLinker
    ...
    def columns(self) -> list[InputColumn]:
        ...
        return [InputColumn(c, sql_dialect="yellowbrick") for c in cols]

    def validate(self):
        if type(self.physical_name) is not str:
            raise ValueError(
                f"{self.df_name} is not a string dataframe.\n"
                "Yellowbrick Linker requires input data"
        ...
        if len(res) == 0:
            raise ValueError(
                f"{self.physical_name} does not exist in the yellowbrick db provided.\n"
                "Yellowbrick Linker requires input data"
                " to be a string containing the name of a"
                " yellowbrick table that exists in the provided db."
            )
...
class YellowbrickLinker(Linker):
    def __init__(
        self,
        ...
        self._sql_dialect_ = "yellowbrick"
        if not isinstance(engine, Engine):
            raise ValueError(
                "You must supply a sqlalchemy engine " "to create a YellowbrickLinker."
            )
    ...
    def _table_to_splink_dataframe(self, templated_name, physical_name):
        return YellowbrickDataFrame(templated_name, physical_name, self)
    ...
    def _random_sample_sql(
        ...
        if seed:
            ...
            raise NotImplementedError(
                "Yellowbrick does not support seeds in random "
